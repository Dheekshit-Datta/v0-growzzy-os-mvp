...
import { Archive, ChevronDown, ChevronRight, Copy, DollarSign, Megaphone, Pause, Play, Search } from "lucide-react"
...
export default function CampaignsPage() {
  ...
  }, [query, selectedAdAccountId, platformFilter, statusFilter, router, pathname])
    ...
    try {
      if (!force) {
        try {
          ...
          if (raw) {
            const cached = JSON.parse(raw) as { at: number; key: string; value: { campaigns: Campaign[]; trust: AdsManagerTrust; selectedAdAccountId?: string | null } }
    ...
    <DashboardLayout>
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-950">Campaigns</h1>
            <p className="text-sm text-slate-500">Search, review, and manage your campaigns.</p>
          </div>
        ...
        <div className="mb-4 rounded-xl border border-border bg-white p-3 shadow-sm">
          ...
          {selected.size > 0 && (
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-[#1F57F5]/20 bg-[#1F57F5]/5 px-3 py-2 text-sm">
              ...
              <div className="flex flex-wrap gap-2">
                <button disabled={!!trust.hasStaleData} title={trust.hasStaleData ? "Sync campaigns before bulk actions" : "Pause selected campaigns"} onClick={() => bulkAction("PAUSE")} className="btn btn-secondary h-8 px-3 text-xs disabled:opacity-50">Pause</button>
                <button disabled={!!trust.hasStaleData} title={trust.hasStaleData ? "Sync campaigns before bulk actions" : "Resume selected campaigns"} onClick={() => bulkAction("RESUME")} className="btn btn-secondary h-8 px-3 text-xs disabled:opacity-50">Resume</button>
                <button disabled={!!trust.hasStaleData} title={trust.hasStaleData ? "Sync campaigns before bulk actions" : "Decrease selected campaign budgets"} onClick={() => bulkAction("DECREASE_BUDGET", 10)} className="btn btn-secondary h-8 px-3 text-xs disabled:opacity-50">Budget -10%</button>
                <button disabled={!!trust.hasStaleData} title={trust.hasStaleData ? "Sync campaigns before bulk actions" : "Increase selected campaign budgets"} onClick={() => bulkAction("INCREASE_BUDGET", 10)} className="btn btn-secondary h-8 px-3 text-xs disabled:opacity-50">Budget +10%</button>
                <button disabled={!!trust.hasStaleData} title={trust.hasStaleData ? "Sync campaigns before bulk actions" : "Remove selected campaigns"} onClick={() => bulkAction("REMOVE")} className="btn btn-secondary h-8 px-3 text-xs text-red-600 disabled:opacity-50">Remove</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-white shadow-sm">
          {loading ? (
            ...
            <EmptyState
              ...
              description="Launch your first campaign or sync your connected Google Ads account."
                      ...
                      <Fragment key={campaign.id}>
                        <tr className="hover:bg-slate-50">
                          ...
                          <td className="px-3 py-3">
                            ...
                            <div className="text-xs text-slate-500">Updated {relativeTime(campaign.verifiedAt || campaign.syncedAt)}</div>
                          </td>
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2">

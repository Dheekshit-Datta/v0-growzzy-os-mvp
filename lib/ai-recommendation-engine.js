...
function thresholdsForRisk(riskLevel) {
    switch (riskLevel) {
        case "CONSERVATIVE":
...
export function generateAIRecommendations(input) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        const [campaigns, userSettings, workspace] = yield Promise.all([
            prisma.campaign.findMany({
                where: Object.assign(Object.assign({}, verifiedMetricCampaignWhere({ userId: input.userId, workspaceId: input.workspaceId })), (input.adAccountId ? { adAccountId: input.adAccountId } : {})),
                select: {
        ...
        if (!campaigns.length)
            return [];
        const riskLevel = (userSettings === null || userSettings === void 0 ? void 0 : userSettings.riskLevel) || "BALANCED";
        const { pauseSpend, roasScaleMultiplier, poorRoasCeiling, ctrFloor } = thresholdsForRisk(riskLevel);
        const targetRoas = (userSettings === null || userSettings === void 0 ? void 0 : userSettings.targetRoas) || ((userSettings === null || userSettings === void 0 ? void 0 : userSettings.primaryKpi) === "ROAS" ? userSettings === null || userSettings === void 0 ? void 0 : userSettings.kpiTarget : null) || 3.5;
        const targetCpa = ((userSettings === null || userSettings === void 0 ? void 0 : userSettings.primaryKpi) === "CPA" ? userSettings === null || userSettings === void 0 ? void 0 : userSettings.kpiTarget : null) || null;
        ...
        function trendRoasChange(campaignId) {
            ...
            if (priorSpend < 10 || recentSpend < 10)
                return null;
        ...
        for (const campaign of campaigns) {
            ...
            const currentBudget = (_b = (_a = campaign.dailyBudget) !== null && _a !== void 0 ? _a : campaign.budgetAmount) !== null && _b !== void 0 ? _b : null;
...
export function generateAndPersistRecommendations(input) {
    return __awaiter(this, void 0, void 0, function* () {
        const recommendations = yield generateAIRecommendations(input);
        if (!recommendations.length)
            return [];
        const campaignIds = recommendations.map((r) => r.campaignId).filter((id) => id != null);
        ...
        yield prisma.optimizationSuggestion.deleteMany({
            where: {
        ...
        const created = yield prisma.$transaction(recommendations.map((r) => prisma.optimizationSuggestion.create({
            data: {
                workspaceId: input.workspaceId,
                userId: input.userId,
        ...
        yield runAutopilotForWorkspace({ userId: input.userId, workspaceId: input.workspaceId }).catch(() => { });
        return created;
    });
}

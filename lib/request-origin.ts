...
export function isAllowedBrowserMutation(input: {
  ...
  if (!MUTATION_METHODS.has(input.method.toUpperCase()) || !input.pathname.startsWith("/api/")) return true
  if (input.pathname.startsWith("/api/cron/") || input.pathname.startsWith("/api/webhooks/")) return true
...
export function requestPassesSameOrigin(req: Request) {

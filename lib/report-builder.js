...
export class ReportBuilder {
    ...
    addPlatformBreakdownSection() {
        if (!this.config.includePlatformBreakdown)
            return;
        this.checkPageBreak(50);
        this.doc.setFontSize(12);
        this.doc.setTextColor(0, 0, 0);
        ...
        this.doc.setFont(undefined, 'normal');
        this.doc.setTextColor(0, 0, 0);
        Object.entries(this.data.platformBreakdown).forEach(([platform, data]) => {
    ...
    addTopCampaignsSection() {
        if (!this.config.includeTopCampaigns || this.data.campaigns.length === 0)
            return;
        ...
        this.doc.setFont(undefined, 'normal');
        this.doc.setTextColor(0, 0, 0);
        this.data.campaigns.slice(0, 5).forEach((campaign) => {
    ...
    addAIAnalysisSection() {
        if (!this.config.includeAIAnalysis || !this.data.insights)
            return;
    ...
    addRecommendationsSection() {
        if (!this.config.includeRecommendations || !this.data.insights)
            return;
        const insights = typeof this.data.insights === 'string'
            ? JSON.parse(this.data.insights)
            : this.data.insights;
    ...
    addPsychologicalInsightsSection() {
        if (!this.data.psychologicalInsights || this.data.psychologicalInsights.length === 0)
            return;
        this.checkPageBreak(40);
        this.doc.setFontSize(12);
        this.doc.setTextColor(0, 0, 0);

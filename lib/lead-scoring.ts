...
export interface LeadScoringInput {
  ...
  // Engagement signals (populated later via analytics)
  openedEmails?: number
  clickedAds?: number
...
const HIGH_INTENT_SOURCES: Record<string, number> = {
  'referral': 90,
  'organic':  80,
  'direct':   75,
  'google_ads': 70,
...
export function calculateLeadScore(input: LeadScoringInput): ScoreBreakdown {
  ...
  let dataScore = 0
  if (input.email) dataScore += 20
  if (input.phone) dataScore += 15
  if (input.company) dataScore += 25
  if (input.position) dataScore += 15
  if (input.source) dataScore += 10
  if (input.estimatedValue && input.estimatedValue > 0) dataScore += 15
  ...
  if (val >= 100000) valueScore = 100
  else if (val >= 50000) valueScore = 85
  else if (val >= 20000) valueScore = 70
  else if (val >= 10000) valueScore = 55
  else if (val >= 5000) valueScore = 40
  else if (val >= 1000) valueScore = 25
  else if (val > 0) valueScore = 15
  else valueScore = 10 // Unknown value
  ...
  else if (clampedScore >= 55) tier = 'WARM'
  else if (clampedScore >= 30) tier = 'COLD'
  else tier = 'ICE'

  // â”€â”€ Reasoning â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

  const reasons: string[] = []
  if (dataScore >= 70) reasons.push('Complete profile data')
  else if (dataScore < 40) reasons.push('Incomplete profile â€” get more info')
  if (engagementScore >= 50) reasons.push('High engagement signals')
  if (intentScore >= 60) reasons.push('Strong buying intent from source/role')
  if (valueScore >= 70) reasons.push('High estimated deal value')
  if (input.company && input.position) reasons.push('Decision-maker at identified company')

...
export type DetectedAlert = {
  id: string
  metric: "reach" | "ctr" | "spend" | "botScore"
  platform: string
  value: number
  threshold: number
...
function rollingAverage(points: MetricPoint[], index: number, metric: keyof MetricPoint, window = 5) {
...
export function detectAlerts(data: MetricPoint[]): DetectedAlert[] {
